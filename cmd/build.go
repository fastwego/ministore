...
func main() {
	...
	flag.StringVar(&buildType, "type", "ministore", "")
	flag.Parse()
...
func build(group ApiGroup) {
	...
	for _, api := range group.Apis {
		...
		switch {
		case strings.Contains(api.Request, "GET http"):
			_TEST_ARGS_STRUCT_ = `ctx *ministore.Ministore, ` + _GET_PARAMS_
		case strings.Contains(api.Request, "POST http"):
			_TEST_ARGS_STRUCT_ = `ctx *ministore.Ministore, payload []byte`
			if _GET_PARAMS_ != "" {
				_TEST_ARGS_STRUCT_ += `,` + _GET_PARAMS_
			}
		case strings.Contains(api.Request, "POST(@media"):
			_TEST_ARGS_STRUCT_ = `ctx *ministore.Ministore, ` + _UPLOAD_ + ` string` + _PAYLOAD_ + _GET_PARAMS_
		}
...
func _FUNC_NAME_(ctx *ministore.Ministore, payload []byte_GET_PARAMS_) (resp []byte, err error) {
...
func _FUNC_NAME_(ctx *ministore.Ministore_GET_PARAMS_) (resp []byte, err error) {
	return ctx.Client.HTTPGet(api_FUNC_NAME__GET_SUFFIX_PARAMS_)
}
`
var postUploadFuncTpl = commentTpl + `
func _FUNC_NAME_(ctx *ministore.Ministore, _UPLOAD_ string_PAYLOAD__GET_PARAMS_) (resp []byte, err error) {
	r, w := io.Pipe()
